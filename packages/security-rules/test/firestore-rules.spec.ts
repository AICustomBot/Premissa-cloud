...
suite("PERMISSA Firestore rules", () => {
  ...
  async function seed() {
    await testEnv.withSecurityRulesDisabled(async (ctx) => {
      ...
      await setDoc(
        doc(db, "users", PRODUCER),
        userProfile(PRODUCER, "PRODUCER", ORG),
      );
      await setDoc(
        doc(db, "users", REVIEWER),
        userProfile(REVIEWER, "REVIEWER", ORG),
      );
      ...
      await setDoc(
        doc(db, "projects", SEALED_PROJECT),
        project(SEALED_PROJECT, true),
      );
  ...
  describe("payload 11: audit log tampering", () => {
    ...
    it("denies updating an existing audit log", async () => {
      await assertFails(
        updateDoc(
          doc(as(PRODUCER), "projects", PROJECT, "auditLogs", "log-1"),
          {
            action: "REWRITTEN",
          },
        ),
  ...
  describe("clearance status is server-owned", () => {
    it("denies a client creating a pre-cleared entity", async () => {
      await assertFails(
        setDoc(
          doc(as(PRODUCER), "projects", PROJECT, "entities", "self-cleared"),
          {
            id: "self-cleared",
            projectId: PROJECT,
